...
public interface DBStatement {
    ...
    String SHOW_USER_ACTIVITIES_BY_DATE = "SELECT activity.activity_name, user_activity.activity_date, user_activity.activity_duration, user.user_id FROM user_activity LEFT JOIN activity ON activity.activity_id=user_activity.activity_id LEFT JOIN user on user_activity.user_id = user.user_id WHERE user.user_id = 29 && user_activity.activity_date = '2020-12-12';";
