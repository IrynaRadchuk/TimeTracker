...
public interface DBStatement {
    ...
    String SHOW_USER_ACTIVITIES = "SELECT activity.activity_name, user_activity.activity_date, user_activity.activity_duration, user.user_first_name, user.user_last_name FROM user_activity LEFT JOIN activity ON activity.activity_id=user_activity.activity_id LEFT JOIN user on user_activity.user_id = user.user_id;";
